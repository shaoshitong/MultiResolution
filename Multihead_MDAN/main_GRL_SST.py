...
segment7:数据形式(45, 310, 180),对(45, 62, 180, 5)的dim=180进行标准化,然后再进行拼接成(45, 62, 900)
再转换成(45, 900, 62), 再拆分成(135, 300, 62), 再转换为(135, 62, 300)
"""
...
def train(myNet, source_loader, target_loader):
    ...
    lr = 0.01
    optimizer = optim.ASGD(myNet.parameters(), lr=0.005)
    ...
    for source_data, source_label in tqdm.tqdm(source_loader, desc='Train epoch={}'.format(epoch), total=length, leave=True):
        ...
        # total loss
        loss_total = loss_weight[0] * loss_e + \
                     loss_weight[1] * loss_d*0 + \
                     loss_weight[2] * sigma*0
        loss_total.backward()
        optimizer.step()
        #scheduler.step()
        i += 1
...
if __name__ == '__main__':
    ...
    sample_path = 'G:/Alex/SEED_experiment/leave one subject out/segment6/'
    # label = 'label_V.npy'
    ...
    for i in range(total):
        ...
        source_loader = DataLoader(source, batch_size=8, shuffle=True, drop_last=True)
        target_loader = DataLoader(target, batch_size=8, shuffle=True, drop_last=False)
        from MultiResolution.Multihead_MDAN.model import OverallModel
        myNet = OverallModel().cuda()
