from contrast import model_removeGRL
import torch
...
def train(myNet, optimizer, source_data_loader):
    best_acc = -float('inf')
    ...
    for epoch in range(args.nEpoch):
        ...
        # tqdm：设置进度条
        for s_sample, s_label in tqdm.tqdm(source_data_loader):

            data_s, label_s_e = s_sample.cuda(), s_label.cuda()
        ...
        item_pr = 'Train Epoch: [{}/{}], emotion_loss: {:.4f}, epoch{}_Acc: {:.4f}' \
            .format(epoch, args.nEpoch, err_e.item(), epoch, train_acc)
        print(item_pr)
...
if __name__ == '__main__':
    ...
    for i in range(total):
        ...
        target_data_loader = DataLoader(target, batch_size=9, shuffle=True, drop_last=True)
        lr_list = []
        myNet = model_removeGRL.NewModel(in_channel=310, out_channel=128, res_inchannel=256, res_outchannel=128).cuda()
        LR = 0.1
        optimizer = optim.SGD(myNet.parameters(), lr=LR)
        scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=5, gamma=0.9)
        train(myNet, optimizer, source_data_loader)