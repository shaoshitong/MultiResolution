...
def plot_accuracy_statistics(cm_before, cm_after, num_classes, args, target_train_loader):
    ...
    plot_cm(axs[0], cm_before, args,
            label='Before {} (Avg. acc: {:.2f})'.format(args.da_strat, pca_before.mean() * 100),
            num_classes=num_classes)
    plot_cm(axs[1], cm_after, args, label='After {} (Avg. acc: {:.2f})'.format(args.da_strat, pca_after.mean() * 100),
            num_classes=num_classes)
    plot_per_class_diff(axs[2], pca_after, pca_before, num_classes, \
                        target_train_loader, title='SENTRY: Per-class accuracy change')

    fig.suptitle(r'{}$\rightarrow${}-LT ({}): {}'.format(args.source, args.target, args.LDS_type, args.da_strat),
                 fontsize=28, y=1.05)
    plt.tight_layout()
