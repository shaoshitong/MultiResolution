    ...
    for i in range(15):  # subject
        ...
        for j in range(3):  # session
            ...
            for k in range(15):  # film clips
                ...
                # [62, 180, 5]
                signal = signal[:, 0:180, :]
                normal_signal = normalize(signal)
                signal_total[k+j*15, :, :, :] = normal_signal
        signal_total = rearrange(signal_total, 'b c h w -> b c (h w)')
        print(signal_total.shape)
        np.save(sample_path + 'person_%d data' % i, signal_total)


def sample_seed_aug(data_path, sample_path):
    # label为字典形式
    label = sio.loadmat(data_path + 'label.mat')
    # 给数据贴标签-0，1，2
    label = np.array(label['label']) + 1
    ...
    for i in range(15):  # subject
        ...
        for j in range(3):  # session
            ...
            for k in range(15):  # film clips
                ...
                signal_total[k+j*15, :, :, :] = normal_signal
        signal_total = rearrange(signal_total, 'b c h w -> b c (h w)')
        # signal_total = rearrange(signal_total, 'b c h -> b h c')
        # signal_total = signal_total.reshape((-1, 300, 62))
        # signal_total = signal_total.transpose(0, 2, 1)
        np.save(sample_path + 'person_%d data' % i, signal_total)
...
if __name__ == '__main__':
    ...
    if dataset == 'DREAMER':
        ...
        sample_seed_aug(data_path, sample_path)
